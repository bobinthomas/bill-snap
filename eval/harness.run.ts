 ...
 *
 * Run: `npm run eval`  (needs the private image mirror for the image entries;
 * `kind: "text"` entries always run via the regex path).
 *
 * Writes the committed baseline to `eval/reports/latest.json` and exits non-zero
 * when any data-bearing criterion fails — a change ships only if it clears
 * these gates (§5.7).
 */
...
it("replays the golden corpus and writes the §5.7 baseline report", async () => {
  loadDevVars();
  ...
  for (const entry of labels) {
    if (entry.kind === "text" && entry.ocr_text !== undefined) {
      ...
      const outcome = await service.run({ imageBytes: image.bytes, imageMimeType: image.mime, gstRegistered: true });
      scored.push(scoreEntry(entry, outcome));
    }
  }
  ...
  if (failed.length > 0 || duplicateFailed) {
    ...
    const dup = duplicateFailed ? `; duplicate gate false-matches: ${[...duplicateGate.labelMatches, ...duplicateGate.extractionMatches].join(", ")}` : "";
    throw new Error(
...
function printSummary(
  ...
  console.log(`\n§5.7 eval — corpus ${total} entries (${scoredCount} scored, ${skipped} missing image)`);
